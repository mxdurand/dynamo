#### New function: P is for Photosynthesis
DYNAMOP <- function(df = df, id = NULL, norm = FALSE, timestep = 30, PltLength = 100, ITER = 100, relPlt = NULL, plotall = TRUE, savePlot = FALSE)
{
  ...
  mresD <- data.frame("id" = character(0), "N_EXP" = integer(0), "N_SIG" = integer(0), "mTau" = numeric(0), "sdTau" = numeric(0), "mk" = numeric(0), "sdk" = numeric(0),
                      ...
                      "pmax_EXP" = numeric(0), "pmax_SIG" = numeric(0), "sdpmax_SIG" = numeric(0), "int_EXP" = numeric(0), "int_SIG" = numeric(0), stringsAsFactors=FALSE)
  ...
  {
    if (savePlot == TRUE) {
      png(paste0("Fits/CurveFit_DYNAMOP_", i, ".png"), height = 8, width = 12, units = "in", res = 300)
    }
    ...
    down.i <- sub.i[sub.i$DownA == "SS0" | sub.i$DownA == "DYN" | sub.i$DownA == "SS1",]
    up.i <- sub.i[sub.i$UpA == "SS0" | sub.i$UpA == "DYN" | sub.i$UpA == "SS1",]
    
    # Remove NAs
    down.i <- down.i[!is.na(down.i$DownA),]
    up.i <- up.i[!is.na(up.i$UpA),]
    
    # Check if data is present, otherwise go to next curve
    WhatSens <- c("DOWN", "UP")
    if(length(down.i$Photo)==0) {WhatSens <- "UP"}
    if(length(up.i$Photo)==0) {WhatSens <- "DOWN"}
    ...
    for (SENS in WhatSens)
    {
      if (SENS == "DOWN") {
        # If curve goes down
        # Here start is later because we look for min of Photo for NPQ
        curve <- down.i[down.i$DownA == "DYN",]
        maxNPQ <- which.min(head(curve,36)$Photo)
        start <- curve$Photo[maxNPQ]
        end <- mean(down.i[down.i$DownA == "SS1",]$Photo, na.rm=T)
        curve <- curve[(maxNPQ+1):nrow(curve),]
        # If using smoothing, log and spline worked best.
        # y <- c(start, curve$Photo, end)
        # x <- 1:length(y)
        # y2 <- smooth.spline(x, y, spar = 0.5)
        # f <- nls(y ~ a * log(x) + b, start = list(a = 0, b = 0))
        
        # If curve normalized between 0 and 1
        if(norm == TRUE)
        {
          curve$Photo <- (curve$Photo - start) / (end - start)
          start <- 0
          end <- 1
        }
      } else if (SENS == "UP") {
        # If curve goes up
        start <- mean(up.i[up.i$UpA == "SS0",]$Photo, na.rm=T)
        end <- mean(up.i[up.i$UpA == "SS1",]$Photo, na.rm=T)
        curve <- up.i[up.i$UpA == "DYN",]
      ...
      CURVE_EXP <- c(start, curve$Photo, rep(end, times=PltLength))
      CURVE_SIG <- c(rep(start, times=PltLength), curve$Photo, rep(end, times=PltLength))
    ...
    if (nrow(allresD[allresD$id == i,]) > 0){
      ...
      mresD[mtempD+1,2:28] <- c( Nquant_EXPD, Nquant_SIGD, mTauD, sdTauD, mkD, sdkD, mlD, sdlD, LCD, mltD, mg0_EXPD, sdg0_EXPD, mG_EXPD, sdG_EXPD, mg0_SIGD, sdg0_SIGD, mG_SIGD, sdG_SIGD, R2quant_EXPD, R2quant_SIGD, R2quant_EXPD_ALL, R2quant_SIGD_ALL, pmax_EXPD, pmax_SIGD, sdpmax_SIGD, int_EXPD, int_SIGD)
      mresD[mtempD+1,1] <- i
    ...
    if (nrow(allresU[allresU$id == i,]) > 0){
      ...
      # Fill final data frame with results
      mresU[mtempU+1,2:28] <- c(Nquant_EXPU, Nquant_SIGU, mTauU, sdTauU, mkU, sdkU, mlU, sdlU, LCU, mltU, mg0_EXPU, sdg0_EXPU, mG_EXPU, sdG_EXPU, mg0_SIGU, sdg0_SIGU, mG_SIGU, sdG_SIGU, R2quant_EXPU, R2quant_SIGU, R2quant_EXPU_ALL, R2quant_SIGU_ALL, pmax_EXPU, pmax_SIGU, sdpmax_SIGU, int_EXPU, int_SIGU)
      mresU[mtempU+1,1] <- i
